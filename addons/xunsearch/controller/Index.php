...
class Index extends Controller
{
    ...
    public function search()
    {
        ...
        try {
            ...
            foreach ($xs->getAllFields() as $field) {
                ...
                if ($field->isNumeric() && isset($fieldList[$field->name]) && $fieldList[$field->name]['sortable']) {
                    $orderList[$field->name . '_desc'] = (isset($fieldColumn[$field->name]) ? $fieldColumn[$field->name] : $field->name) . "从大到小";
                    $orderList[$field->name . '_asc'] = (isset($fieldColumn[$field->name]) ? $fieldColumn[$field->name] : $field->name) . "从大到小";
                }
                if ($field->isSpeical()) {
                    continue;
                }
            ...
            // 设置排序
            if (strrpos($order, '_') !== false) {
                list($sf, $st) = explode('_', $order);
        ...
        $template = $project['listtpl'] ? $project['listtpl'] : "list";
        $this->view->assign("title", "搜索 {$q} 的结果 - {$project['title']}");

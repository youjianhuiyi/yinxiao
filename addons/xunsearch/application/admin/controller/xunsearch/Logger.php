...
class Logger extends Backend
{
    ...
    /**
     * Logger模型对象
     * @var \app\admin\model\xunsearch\Logger
     */
    ...
    public function index()
    {
        ...
        if ($this->request->isAjax()) {
            ...
            try {
                ...
                $result = $xs->search->setDb(\XSSearch::LOG_DB)->setLimit($limit, $offset)->search($query ? $query : 'total:1');
    ...
    public function add()
    {
        if ($this->request->isPost()) {
            ...
            $nums = isset($row['nums']) ? $row['nums'] : 1;
            if (!$word || !$nums) {
    ...
    public function edit($ids = null)
    {
        ...
        if ($this->request->isPost()) {
            $row = $this->request->post("row/a", []);
            $word = isset($row['word']) ? $row['word'] : '';
            $nums = isset($row['nums']) ? $row['nums'] : 0;
            if (!$word) {
                $this->error("搜索词和搜索次数不能为空");
            }
            $search->addSearchLog($word, $nums);
